...
bool Load::load(tf::Vector3& origin, tf::Quaternion& q,std::string path_to_load){

    std::string folder_load = path_to_load;

    if( !(boost::filesystem::exists(folder_load))){
        std::cerr << "FAILED to open folder: " + path_to_load << "\n";
        return false;
    }else{
